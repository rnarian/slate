...
SettingsDialog.prototype.render = function(){
  ...
  renderedConnections = connections.map(function(connectionData){
  ...
  var html = dialog({ connections: renderedConnections });
  return dom(html);
};
...
SettingsDialog.prototype.show = function(){
  ...
  el.on('click', '.add-channel', function(e){
    e.preventDefault();
    // TODO: add it to the correct connection
    self.addChannel(settings.connections[0]);
  });
